...
def job_wakeup():
    ...
    url = 'https://mx-cron2.herokuapp.com/'
    r = requests.get(url)
    print(r)
...
def job_function2():
    ...
    for item in records:
        if item['County']=='高雄市' and item['SiteName']=='鳳山':
            send_line('[MAXTEST-wakup-auto_10:15]%s>> AQI=%s' %(item['SiteName'], item['AQI']))
...
def start_scheduler():
    ...
    # 每天早上6:30執行
    scheduler.add_job(job_function2, 'cron', hour='10', minute='15')
